...
function! s:Project(filename) " <<<
	...
	syntax match projectScriptinEncoding    '\<encoding\s*=.*"' contains=projectDescription,projectWhiteError
	...
	" Add by Wangfc
	" Extract open  
	let cmd_open = s:GetScriptOpen(infoline,home)
	if cmd_open[0]!=''
            let cmd_open=' open='.cmd_open
	endif
	let encoding = s:GetEncoding(infoline)
    if encoding==''
        let encoding=s:GetEncoding(parent_infoline)
    endif
	if encoding[0]!=''
		let encoding=' encoding="'.encoding.'"'
	endif

        let ret_value=s:ConstructInfo(home, c_d, scriptin, scriptout, '', filter)
	return ret_value.' '.cmd_open.encoding
    endfunction ">>>
    ...
    function! s:OpenEntry2(line, infoline, fname, editcmd)
        ...
        "echo a:infoline
        "Add by Wangfc
	let open_cmd = s:GetScriptOpen(a:infoline,home)
	"echo '----------------'.open_cmd
        if open_cmd != ''
            if !filereadable(glob(open_cmd))
                call confirm('"'.open_cmd.'" not found. Ignoring.', "&OK", 1)
            else
		"echo ' this wangfc '.open_cmd
                exec 'source '.open_cmd
            endif
        endif
	    ...
	    let encoding = s:GetEncoding(a:infoline)
	    if strlen(encoding)>0
            ...
            if s:IsAbsolutePath(fname) == 2
                exec a:editcmd.' '.encoding.' '.fname
            else
                " echo 'silent '.a:editcmd.' '.g:proj_open_opt.' '.fname
		"echo a:infoline
		"echo 'silent '.a:editcmd.' '.encoding.' '.fname
                silent exec 'silent '.a:editcmd.' '.encoding.' '.fname
            endif
    ...
    function! s:GetEncoding(info)
		let encode=substitute(a:info,'.*\<encoding="\([^"]*\).*','\1','')
		if strlen(encode)==strlen(a:info) 
		   	return "" 
	   	endif
		return encode	
    endfunction
    ...
    function! s:GetScriptOpen(info, home)
        let scriptin = substitute(a:info, '.*\<open=\(\(\\ \|\f\|:\)\+\).*', '\1', '')
        if strlen(scriptin) == strlen(a:info)
            let scriptin=substitute(a:info, '.*\<open="\(.\{-}\)".*', '\1', '')
            if strlen(scriptin) != strlen(a:info) | let scriptin=escape(scriptin, ' ') | endif
        endif
        if strlen(scriptin) == strlen(a:info) | let scriptin='' | else
        if !s:IsAbsolutePath(scriptin) | let scriptin=a:home.'/'.scriptin | endif | endif
